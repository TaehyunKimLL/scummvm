...
public:
	void test_empty_map_defaults() {
		...
		for (uint i = 0; i < ARRAYSIZE(loaded); ++i) {
			const FontSettings s = resolveFontSettings(empty, loaded[i], 0, noIni, gameDir());
		...
		const Graphics::HiResTextConfig full = parse(
			"[hires]\nfont=/f/a.ttf\nsize=20\n[latin]\nmode=half\n[font.0]\nsize=12\n");
		const FontSettings s = resolveFontSettings(full, false, 0, noIni, gameDir());
		TS_ASSERT(s.facePath.empty());
		TS_ASSERT_EQUALS(s.size, 16);
		TS_ASSERT_EQUALS(s.latin, Sci::kLatinOff);
	}
	...
	void test_font_section_overrides_latin_section() {
		...
		FontSettings s = resolveFontSettings(map, true, 4, noIni, gameDir());
		...
		s = resolveFontSettings(map, true, 300, noIni, gameDir());
	...
	void test_platform_section_wins_on_its_platform_only() {
		...
		FontSettings s = resolveFontSettings(pc98, true, 0, noIni, gameDir());
		TS_ASSERT_EQUALS(s.latin, Sci::kLatinFullwidth);
		TS_ASSERT_EQUALS(s.size, 18);
		s = resolveFontSettings(pc98, true, 4, noIni, gameDir());
		TS_ASSERT_EQUALS(s.latin, Sci::kLatinProportional);

		// On DOS neither qualified section applies.
		const Graphics::HiResTextConfig dos = parse(text, "dos");
		s = resolveFontSettings(dos, true, 0, noIni, gameDir());
		TS_ASSERT_EQUALS(s.latin, Sci::kLatinHalf);
		TS_ASSERT_EQUALS(s.size, 14);
		s = resolveFontSettings(dos, true, 4, noIni, gameDir());
		TS_ASSERT_EQUALS(s.latin, Sci::kLatinHalf);
		TS_ASSERT_EQUALS(s.size, 16);
	}
	...
	void test_ini_overrides_map() {
		...
		for (uint i = 0; i < ARRAYSIZE(ids); ++i) {
			const FontSettings s = resolveFontSettings(map, true, ids[i], ini, gameDir());
		...
		const FontSettings s = resolveFontSettings(map, true, 4, sizeOnly, gameDir());
		...
		const FontSettings t = resolveFontSettings(none, false, 0, relative, gameDir());
	...
	void test_enabled_true_means_proportional_game() {
		...
		map = parse("[latin]\nenabled=true\n[font.4]\nlatin=off\n");
		s = resolveFontSettings(map, true, 4, noIni, gameDir());
		TS_ASSERT_EQUALS(s.latin, Sci::kLatinOff);
	...
	void test_relative_face_path_joins_game_dir() {
		const Graphics::HiResTextConfig map = parse(
			"[hires]\n"
			"font=default\n"
			"[fonts]\n"
		...
		// A [fonts] name whose file is relative lands in the game directory;
